...
void main()
{
    ...
    for(int i = 0; i < MAX_LIGHTS; i++)
    {
        if(Lights[i].Type == 0)
        {
            ...
            resultColor += vec3(Color.rgb * Lights[i].Intensity * (ambient + attenuation * (diffuse + specular)));
        }
        else
        {
    ...
    FragColor = vec4(texture(DiffuseTexture, FragUV + Time * 0.05f).rgb * resultColor, Color.w);
}
